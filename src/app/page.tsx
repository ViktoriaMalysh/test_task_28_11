...
export default function Home() {
  ...
  const { mutate: assignPerson, isPending } = useAssignPeople();
  ...
  return (
    <div className="font-sans flex flex-col gap-4">
      <div className="bg-neutral-200 border-none px-8 py-4 text-2xl font-semibold">
        People{" "}
        <span className="border border-neutral-300 px-2">{people.length}</span>
      </div>
      <div>
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 px-8 text-gray-700">
              ...
              >
                ...
                {isEditPerson.id && isEditPerson.id === person.id ? (
                  <div className="flex gap-4">
                    ...
                    >
                      {rolesList.map((role: IOption, indexRole: number) => (
                        <option key={indexRole} value={role.value}>
                          {role.label}
                        </option>
                      ))}
                    </select>
                  ...
                  <div className="flex justify-start items-center gap-4">
                    ...
                    <button
                      onClick={() =>
                        setIsEditPerson({ newRole: person.role, id: person.id })
                      }
                      disabled={isPending}
                      className="border border-gray-500 text-gray-700 px-3 py-1 rounded"
                    >
